...
export default class PatternDetectionService {
    ...
    async loadSourceImages() {
        try {
            ...
            const response = await fetch(process.env.PUBLIC_URL + '/images-source/images.json');
            if (!response.ok) {
                throw new Error(`Erreur HTTP: ${response.status}`);
            }
            
            const data = await response.json();
            this.sourceImages = data.images.map(img => process.env.PUBLIC_URL + img.path);
    ...
    async initializeOpenCVOnce() {
        ...
        PatternDetectionService.opencvLoadPromise = new Promise((resolve, reject) => {
            ...
            script.src = process.env.PUBLIC_URL + '/opencv/opencv.js';  // Chemin corrigé
            script.async = true;
    ...
    async initLogoTemplate() {
        return new Promise((resolve, reject) => {
            try {
                ...
                img.src = process.env.PUBLIC_URL + '/images-source/logoUS.png'; 
            } catch (error) {
    ...
    async detectPattern(imageData) {
        try {
            ...
            const results = await this.compareImages(imageData);
            const bestMatch = results.reduce((best, current) => {
                return (current.similarity > best.similarity) ? current : best;
            }, { similarity: 0 });

            return {
                matchFound: bestMatch.similarity > 50,
                confidence: bestMatch.similarity,
                fileName: bestMatch.sourceImage,
                commonFeatures: bestMatch.matches
            };
        } catch (error) {
            console.error('Erreur dans detectPattern:', error);
            throw error;
        }
    }
